...
export default function GradesManagement({ user, onNavigate }: { user: any, onNavigate?: (menu: string) => void }) {
  ...
  useEffect(() => {
     ...
     const fetchAssociatedTPs = async () => {
        ...
        const gradeLevel = cls ? cls.level : ''
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 px-2">
         {isLoadingMetadata ? (
            ...
            <>
                ...
                <Card className="rounded-[2.5rem] border-none shadow-xl bg-white p-7 text-left hover:shadow-2xl hover:translate-y-[-4px] transition-all duration-300 group">
                    <div className="flex items-center justify-between mb-4">
                        <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Jenis Nilai</Label>
                        <Filter className="w-4 h-4 text-gray-300 group-hover:text-brand-purple transition-colors" />
                    </div>
                    ...
                    <SelectContent className="rounded-2xl border-none shadow-2xl">
                        <SelectItem value="MIDTERM" className="rounded-xl my-1">PTS (Tengah Semester)</SelectItem>
                        <SelectItem value="FINAL" className="rounded-xl my-1">PAS (Akhir Semester)</SelectItem>
                    </SelectContent>
                    </Select>
                </Card>
